...
% 1) ../third_party/Superpixels/SourceCode/segment/segmentImgOpt.cpp
  mex(fullfile(progroot, 'third_party/Superpixels/SourceCode/segment/segmentImgOpt.cpp'));
%   system('mv ../third_party/Superpixels/SourceCode/segment/segmentImgOpt.mex* ./bin');

% 2) ./Debug/SparseAverageSample2DOptimized.cpp
  mex(fullfile(filepath, 'Features/SparseAverageSample2DOptimized.cpp'));
...
addpath(genpath(fullfile(progroot, 'LearningCode/')));
addpath(genpath(fullfile(progroot, 'LaserDataCollection/')));
...
%   addpath('/afs/cs/group/reconstruction3d/Data/cvx');
   addpath(fullfile(progroot, 'third_party/opt/cvx'));
   cvx_setup;
end
...
%   addpath(genpath('/afs/cs/group/reconstruction3d/Data/yalmip'));
   addpath(genpath(fullfile(progroot, 'third_party/opt/yalmip')));
end

if SeDuMiFlag
   % add sedumi
%   path(path,'/afs/cs/group/reconstruction3d/Data/SeDuMi_1_1R3/SeDuMi_1_1');
   addpath(genpath(fullfile(progroot, 'third_party/opt/SeDuMi_1_1R3')) );
end

if LMIrankFlag
   % add sedumi
%   path(path,'/afs/cs/group/reconstruction3d/Data/SeDuMi_1_1R3/SeDuMi_1_1');
   path(path, fullfile(progroot, 'third_party/opt/lmirank'));
end

function [ SupNeighborTable ] = FixSupNeighbours( Default, fgmaskidx, SupNeighborTable, Sup2Para, sup, SupOri  )
%lr_sups stores the SPs on the left and right of each new SP.
        ...
        for idx = 1:size(h_divs,2)
            ...
            for regidx = 1:numlabels
                ...
                for r_idx = 1:length(valid_rows)
                    ...
                    if (lr_sups_curr(r_idx,1) ~= -1) && (lr_sups_curr(r_idx,2) ~= -1)
                        SupNeighborTable( lr_sups_curr(r_idx,1), lr_sups_curr(r_idx,2) ) = 1;
                end
